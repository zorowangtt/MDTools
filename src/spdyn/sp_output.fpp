...
module sp_output_mod
  ...
  ! valiables
  real(dp),         private, allocatable :: tmp_coord1(:,:)
  real(dp),         private, allocatable :: tmp_coord2(:,:)
  real(dp),         private, allocatable :: tmp_coord3(:,:,:)
...
contains
  ...
  subroutine output_md(output, dynamics, boundary, pairlist, &
    ...
    if (dynamics%crdout_period > 0) then
      if (mod(istep,dynamics%crdout_period) == 0) then
        ...
        tmp_coord(1:3,1:MaxAtom,1:ncell) = 0.0_dp
      ...
      if (mod(istep,dynamics%velout_period) == 0) then
        ...
        if (dynamics%integrator == IntegratorLEAP) then
          if (ensemble%tpcontrol == TpcontrolLangevin) then
            scale_v = sqrt(1.0_dp+0.5_dp*gamma_t*dt)
          else
            scale_v = 1.0_dp
          end if
  ...
  subroutine output_restart_md(output, domain, dynamics, boundary, dynvars)
    ...
    ! reduce coordinates
    !
    tmp_coord1(1:3,1:natom_all) = 0.0_dp
    do i = 1, ncell
      do ix = 1, natom(i)
  ...
  subroutine write_trajectory_dcd(output, boundary, domain, istep, nstep, &
    ...
    real(dp),                parameter     :: TIMEFAC = 48.88821
    ...
    if (output%out_dcd_header) then
      ...
      ts_namd = real(timestep * 1000_dp / TIMEFAC, sp)
  ...
  subroutine reduce_coord(coord, temporary, natom)
    ...
    real(dp),                intent(inout) :: temporary(:,:)
