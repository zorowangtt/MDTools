...
contains
  ...
  subroutine compute_energy_restraints(get_coord, calc_force, domain, boundary,   &
                                       enefunc, coord, force, virial, virial_ext, &
                                       eposi, ermsd, rmsd, ebonds, eemfit, emcorr)
    ...
    ! local variables
    real(wp)                 :: edist, eangle, edihed, etilt, eexp, erepul, efb
  ...
  subroutine compute_energy_restraints_pos(calc_force, domain, enefunc, coord, &
    ...
    do i = id+1, ncell_local, nthread
      ...
      do ix = 1, nrestraint(i)
        ...
        ! write(*,*)'x', restraint_coord(1,ix,i), coord(1,i1,i), rotated_coord(1,i1,i)
        ! write(*,*)'y', restraint_coord(2,ix,i), coord(2,i1,i), rotated_coord(2,i1,i)
        ! write(*,*)'z', restraint_coord(3,ix,i), coord(3,i1,i), rotated_coord(3,i1,i)
        ...
        if ((enefunc%fitting_method == FittingMethodTR_ROT) .or. &
          ...
          d2(1:3)      = real(tmp_wp(1:3),dp)
        end if

        ! check too big energy
        !
        if (etmp > enemax) then
          call error_msg('Compute_Energy_Restraints_Pos> Positional restraint energy is too big (see "Chapter: Trouble shooting" in the user manual)')
        end if
  ...
  subroutine compute_energy_restraints_rmsd(domain, enefunc, coord, &
                                            force, virial, virial_ext, ermsd, rmsd)
  ...
  subroutine compute_energy_restraints_angle(calc_force, enefunc, angle_coord, &
    ...
    integer                  :: inum, i, j, k, jx, index, ind
    integer                  :: iatm
    logical                  :: anglecheck
