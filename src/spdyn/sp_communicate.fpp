...
module sp_communicate_mod
  ...
  type, public :: s_comm  
    ...
    real(dp),         allocatable :: buf_send(:,:)
    real(dp),         allocatable :: buf_recv(:,:)
  end type s_comm  
  ...
  public  :: update_cell_boundary_table
  public  :: update_cell_boundary_constraints
...
contains
  ...
  subroutine communicate_force(domain, comm, force)
    ...
    do ii = 3, 1, -1
      ...
      call mpi_isend(buf_send(1,1), 3*lower_send(ii), mpi_real8, &
                     iproc_lower(ii),                            &
                     my_city_rank,  &
                     mpi_comm_city, irequest1, ierror)
      ...
      ! send the data (upper)
      !
      call mpi_irecv(buf_recv(1,2), 3*lower_recv(ii), mpi_real8, &
                     iproc_lower(ii),                            &
                     my_city_rank+nproc_city,  &
                     mpi_comm_city, irequest2, ierror)
      call mpi_isend(buf_send(1,2), 3*upper_send(ii), mpi_real8, &
                     iproc_upper(ii),                            &
                     iproc_upper(ii)+nproc_city,  &
                     mpi_comm_city, irequest3, ierror)
  ...
  subroutine communicate_coor(domain, comm)
    ...
    real(dp),        pointer :: coord(:,:,:), buf_send(:,:), buf_recv(:,:)
    ...
    do ii = 3, 1, -1
      ...
      do i = 1, num_cell(ii)
        ...
        do ix = 1, ptl_add(ic)
          ...
          do list = 1, 2
            list1 = 2*(k-1) + list + size*num_cell(ii)
        ...
        do j = 1, constraints%connect
          do ix = 1, HGr_move(j,ic)
            do l = 1, j+1
              ...
              do list = 1, 2
                list1 = 2*(k-1) + list + int_size
                int_send(list1,1) = HGr_move_int(2*(l-1)+list,ix,j,ic)
              end do
        ...
        do j = 1, constraints%connect
          do ix = 1, HGr_move(j,ic)
            ...
            do l = 1, j+1
              ...
              do list = 1, 2
                list1 = 2*(k-1) + list + int_size
                int_send(list1,2) = HGr_move_int(2*(l-1)+list,ix,j,ic)
              end do
        ...
        do j = 1, constraints%connect
          ...
          do ix = HGr_move(j,ic)+1, HGr_move(j,ic)+nadd
            ...
            do l = 1, j+1
              ...
              do list = 1, 2
                list1 = 2*(k-1) + list + int_size
                HGr_move_int(2*(l-1)+list,ix,j,ic) = int_recv(list1,1)
              end do
        ...
        do j = 1, constraints%connect
          ...
          do ix = HGr_move(j,ic)+1, HGr_move(j,ic)+nadd
            do l = 1, j+1
              ...
              do list = 1, 2
                list1 = 2*(k-1) + list + int_size
                HGr_move_int(2*(l-1)+list,ix,j,ic) = int_recv(list1,2)
              end do
  ...
  subroutine communicate_contact(domain, comm, enefunc)
    ...
    real(wp),         pointer :: buf_rb_dihed_real(:,:,:)
    real(wp),         pointer :: buf_restraint_real(:,:,:)
    real(wp),         pointer :: buf_contact_real(:,:,:)
    integer,          pointer :: if_lower_send(:,:), if_upper_send(:,:)
    integer,          pointer :: if_lower_recv(:,:), if_upper_recv(:,:)
    integer,          pointer :: iproc_upper(:), iproc_lower(:)
    integer,          pointer :: int_send(:,:), int_recv(:,:)
    integer,          pointer :: bond_add(:), buf_bond_integer(:,:,:)
    integer,          pointer :: contact_add(:), buf_contact_integer(:,:,:)
    integer,          pointer :: angle_add(:), buf_angle_integer(:,:,:)
    integer,          pointer :: dihed_add(:), buf_dihed_integer(:,:,:)
    integer,          pointer :: rb_dihed_add(:), buf_rb_dihed_integer(:,:,:)
    integer,          pointer :: impr_add(:), buf_impr_integer(:,:,:)
    ...
    buf_contact_real      => enefunc%buf_contact_real
    buf_contact_integer   => enefunc%buf_contact_integer
    angle_add             => enefunc%angle_add
    buf_angle_real        => enefunc%buf_angle_real
    buf_angle_integer     => enefunc%buf_angle_integer
    dihed_add             => enefunc%dihed_add
    ...
    do ii = 3, 1, -1
      ...
      k = 7*comm%num_cell(ii)
      j = 0

      do i = 1, comm%num_cell(ii)

        ic = if_lower_send(i,ii)
        int_send(7*i-6,1) = bond_add(ic)
        int_send(7*i-5,1) = angle_add(ic)
        int_send(7*i-4,1) = dihed_add(ic)
        int_send(7*i-3,1) = rb_dihed_add(ic)
        int_send(7*i-2,1) = impr_add(ic)
        int_send(7*i-1,1) = contact_add(ic)
        int_send(7*i  ,1) = restraint_add(ic)
      ...
      do i = 1, comm%num_cell(ii)
        ...
        ic = if_upper_send(i,ii)
        int_send(7*i-6,2) = bond_add(ic)
        int_send(7*i-5,2) = angle_add(ic)
        int_send(7*i-4,2) = dihed_add(ic)
        int_send(7*i-3,2) = rb_dihed_add(ic)
        int_send(7*i-2,2) = impr_add(ic)
        int_send(7*i-1,2) = contact_add(ic)
        int_send(7*i  ,2) = restraint_add(ic)
      ...
      send_size(2,2) = j
       ...
       !
       call mpi_irecv(recv_size(1,1), 2, mpi_integer, iproc_upper(ii),     &
                      iproc_upper(ii)*nproc_city + my_city_rank,           &
                      mpi_comm_city, irequest, ierror)
       call mpi_isend(send_size(1,1), 2, mpi_integer, iproc_lower(ii),     &
                      my_city_rank*nproc_city + iproc_lower(ii),           &
                      mpi_comm_city, irequest1, ierror)
       call mpi_irecv(recv_size(1,2), 2, mpi_integer, iproc_lower(ii),     &
                      my_city_rank*nproc_city + iproc_lower(ii),           &
                      mpi_comm_city, irequest2, ierror)
  ...
  subroutine update_cell_boundary(domain, comm, boundary)
    ...
    do ii = 1, 3
      ...
      do i = 1, num_cell(ii)
        ...
        do ix = 1, natom(ic)
          ...
          int_send(2*k-1,1) = atmcls(ix,ic)
          int_send(2*k,1)   = id_l2g(ix,ic)
        end do
      ...
      do i = 1, num_cell(ii)
        ...
        do ix = 1, natom(ic)
          ...
          int_send(2*k-1,2) = atmcls(ix,ic)
          int_send(2*k,2)   = id_l2g(ix,ic)
        end do
       ...
       do i = 1, num_cell(ii)
         ...
         do ix = 1, natom(ic)
           ...
           atmcls(ix,ic)     = int_recv(2*k-1,1)
           id_l2g(ix,ic)     = int_recv(2*k,1)
       ...
       do i = 1, num_cell(ii)
         ...
         do ix = 1, natom(ic)
           ...
           atmcls(ix,ic)     = int_recv(2*k-1,2)
           id_l2g(ix,ic)     = int_recv(2*k,2)
           id_g2l(1,id_l2g(ix,ic)) = ic
           id_g2l(2,id_l2g(ix,ic)) = ix
          end do
  ...
  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    updae_cell_boundary_table
  ...
  subroutine update_cell_boundary_table(enefunc, domain, comm, boundary)
  ...
  end subroutine update_cell_boundary_table
  ...
  subroutine update_cell_boundary_constraints(domain, comm, boundary, &
                                              constraints)
  ...
  end subroutine update_cell_boundary_constraints
  ...
  subroutine update_cell_size(domain, comm)
    ...
    do ii = 1, 3
      ...
      call mpi_irecv(recv_size(1,1), num_cell(ii), mpi_integer,iproc_upper(ii),&
                     iproc_upper(ii),                &
                     mpi_comm_city, irequest, ierror)
      call mpi_isend(send_size(1,1), num_cell(ii), mpi_integer,iproc_lower(ii),&
                     my_city_rank,                &
                     mpi_comm_city, irequest1, ierror)
      call mpi_irecv(recv_size(1,2), num_cell(ii), mpi_integer,iproc_lower(ii),&
                     my_city_rank+nproc_city,                &
                     mpi_comm_city, irequest2, ierror)
      call mpi_isend(send_size(1,2), num_cell(ii), mpi_integer,iproc_upper(ii),&
                     iproc_upper(ii)+nproc_city,                &
                     mpi_comm_city, irequest3, ierror)
    ...
    do ii = 3, 1, -1
      ...
      if (upper_frecv(ii) /= k) &
        call error_msg('Update_Cell_Size> Disagreement between sending'//&
                       'and receving processors')
  ...
  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    update_cell_size_solute
  !> @brief        Update cell size with solute
  !! @authors      JJ
  !! @param[inout] domain : domain information
  !! @param[inout] comm   : communication information
  !
  ...
  subroutine update_cell_size_solute(domain, comm)
    ...
    integer,         pointer :: num_cell(:), natom(:), nsolute(:)
    integer,         pointer :: if_lower_send(:,:), if_upper_send(:,:)
    integer,         pointer :: if_lower_recv(:,:), if_upper_recv(:,:)
    integer,         pointer :: ic_lower_send(:,:), ic_upper_send(:,:)
    integer,         pointer :: ic_lower_recv(:,:), ic_upper_recv(:,:)
    ...
    size = max(num_cell(1), num_cell(2), num_cell(3))
    allocate(recv_size(2*size,2))
    allocate(send_size(2*size,2))
    ...
    do ii = 1, 3
      ...
      do i = 1, num_cell(ii)
        ic = ic_lower_send(i,ii)
        send_size(2*i-1,1) = natom(ic)
        send_size(2*i,1) = nsolute(ic)
        k = k + natom(ic)
      end do
      lower_send(ii) = k
      ...
      do i = 1, comm%num_cell(ii)
        ic = ic_upper_send(i,ii)
        send_size(2*i-1,2) = natom(ic)
        send_size(2*i,2) = nsolute(ic)
        k = k + natom(ic)
      end do
      upper_send(ii) = k
      ...
      ! send the size of the data
      !
      call mpi_irecv(recv_size(1,1), 2*num_cell(ii), mpi_integer,      &
                     iproc_upper(ii),                                  &
                     iproc_upper(ii),        &
                     mpi_comm_city, irequest, ierror)
      call mpi_isend(send_size(1,1), 2*num_cell(ii), mpi_integer,      &
                     iproc_lower(ii),                                  &
                     my_city_rank,        &
                     mpi_comm_city, irequest1, ierror)
      call mpi_irecv(recv_size(1,2), 2*num_cell(ii), mpi_integer,      &
                     iproc_lower(ii),                                  &
                     my_city_rank+nproc_city,        &
                     mpi_comm_city, irequest2, ierror)
      call mpi_isend(send_size(1,2), 2*num_cell(ii), mpi_integer,      &
                     iproc_upper(ii),                                  &
                     iproc_upper(ii)+nproc_city,        &
                     mpi_comm_city, irequest3, ierror)
      ...
      do i = 1, num_cell(ii)
        ic = ic_upper_recv(i,ii)
        natom(ic) = recv_size(2*i-1,1)
        nsolute(ic) = recv_size(2*i,1)
        k = k + natom(ic)
      end do
      upper_recv(ii) = k
      ...
      do i = 1, num_cell(ii)
        ic = ic_lower_recv(i,ii)
        natom(ic) = recv_size(2*i-1,2)
        nsolute(ic) = recv_size(2*i,2)
        k = k + natom(ic)
      end do
      lower_recv(ii) = k
    ...
    do ii = 3, 1, -1
      ...
      if (upper_frecv(ii) /= k) &
        call error_msg('Update_Cell_Size_Solute> Disagreement between'//&
                       ' sending and receving processors')
  ...
  end subroutine update_cell_size_solute
  ...
  subroutine update_random_number(domain, comm)
    ...
    do ii = 1, 3
      ...
      ! send the data
      !
      call mpi_irecv(buf_recv(1,1), comm%num_cell(ii), mpi_real8,          &
                     iproc_upper(ii),                                      &
                     iproc_upper(ii),            &
                     mpi_comm_city, irequest, ierror)
      call mpi_isend(buf_send(1,1), comm%num_cell(ii), mpi_real8,          &
                     iproc_lower(ii),                                      &
                     my_city_rank,            &
                     mpi_comm_city, irequest1, ierror)
      ...
      ! send the data(upper)
      !
      call mpi_irecv(buf_recv(1,2), comm%num_cell(ii), mpi_real8,          &
                     iproc_lower(ii),                                      &
                     my_city_rank+nproc_city,            &
                     mpi_comm_city, irequest2, ierror)
      call mpi_isend(buf_send(1,2), comm%num_cell(ii), mpi_real8,          &
                     iproc_upper(ii),                                      &
                     iproc_upper(ii)+nproc_city,            &
                     mpi_comm_city, irequest3, ierror)
