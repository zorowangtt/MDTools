...
contains
  ...
  subroutine setup_rpath(rpath_info, sel_info, rst, rstmep, dynamics, &
                         molecule, restraints, enefunc, minimize, dynvars, rpath)
  ...
  subroutine reparametrize(rpath)
    ...
    if (main_rank) then
      ...
      do repid_i = 2, rpath%nreplica - 1
        do dimno = 1, rpath%dimension
          path_smooth(repid_i,dimno) = (1.0_dp-rpath%smooth) * path(repid_i,dimno) + &
            (rpath%smooth*0.5_dp)*(path(repid_i-1,dimno) + path(repid_i+1,dimno))
        end do
      end do
      ...
      path_equi = 0.0_dp
      do repid_i = 2, rpath%nreplica
        path_equi(repid_i) = (repid_i-1)*path_leng(rpath%nreplica)/real((rpath%nreplica -1),wp)
      end do
