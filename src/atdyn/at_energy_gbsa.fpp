...
contains
  ...
  subroutine compute_energy_gbsa(enefunc, molecule, pairlist, nonb_ene, &
                                         coord, force, virial, esolv)
    ...
    call compute_energy_gb(enefunc, molecule, pairlist, coord, force, &
                           virial, esolv)
    ...
    call compute_energy_sa(enefunc, molecule, pairlist, coord, force, &
                           virial, esolv)
  ...
  subroutine compute_energy_gb(enefunc, molecule, pairlist, coord, force, &
                               virial, esolv)
      ...
      do k = ini_all, fin_all
        ...
        if (rij2 < cutoff2) then
          ...
          if (rij > cutoff - rho_is) then
            ...
            Hij(j) = Hij(j) + 0.125_wp*(term1 + term2 + term3)/rij
          else if (rij > 4.0_wp*rho_is) then
            tmp    = rho_is2/rij2
            term1  = pa + tmp*(pb + tmp*(pc + tmp*(pd + tmp*pe)))
            term2  = tmp*tmp/rho_is
            Hij(j) = Hij(j) + term1*term2
          else if (rij > rho_j0 + rho_is) then
            term1  = rho_is/(rij2 - rho_is2)
            term2  = 0.5_wp*log((rij - rho_is)/(rij + rho_is))/rij
            Hij(j) = Hij(j) + 0.5_wp*(term1 + term2)
          else if (rij > abs(rho_j0 - rho_is)) then
            term1  = 2.0_wp - 1.0_wp/(2.0_wp*rij*rho_j0)*(rij2+rho_j02-rho_is2)
            term1  = term1/rho_j0
            term2  = - 1.0_wp/(rij + rho_is)
            term3  = 1.0_wp/rij*log(rho_j0/(rij+rho_is))
            Hij(j) = Hij(j) + 0.25_wp*(term1 + term2 + term3)
          else if (rho_j0 < rho_is) then
            term1  = rho_is/(rij2 - rho_is2)
            term2  = 2.0_wp/rho_j0
            term3  = 1.0_wp/(2.0_wp*rij)*log((rho_is-rij)/(rho_is+rij))
            Hij(j) = Hij(j) + 0.5_wp*(term1 + term2 + term3)
          else
            Hij(j) = Hij(j) + 0.0_wp
          end if
...
!    !$omp reduction(+:force) reduction(+:esolv_tmp)
  ...
  subroutine compute_energy_sa(enefunc, molecule, pairlist, coord, force, &
                               virial, esolv)
