...
contains
  ...
  subroutine setup_pme_grid_number(ene_info, boundary, enefunc,  &
    ...
    !
    if ((ene_info%pme_ngrid_x .ne. ngrid(1)) .or. &
        (ene_info%pme_ngrid_y .ne. ngrid(2)) .or. &
        (ene_info%pme_ngrid_z .ne. ngrid(3))) then
      if (main_rank) then
        write(MsgOut,'(A)') &
          ''
        if (ene_info%pme_ngrid_x.eq.0 .and. ene_info%pme_ngrid_y.eq.0 .and. &
            ene_info%pme_ngrid_z.eq.0) then
          write(MsgOut,'(A)') &
