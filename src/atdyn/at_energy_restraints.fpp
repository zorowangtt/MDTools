...
contains
  ...
  subroutine compute_energy_positional_restraint(enefunc, coord, inum, &
    ...
    if (enefunc%rpath_flag .and. enefunc%rpath_pos_func .gt. 0 ) then
      ...
      case (FittingMethodTR_ROT)
        ...
        if (enefunc%fitting_move .eq. FittingMoveSYS) then
        ...
        else if (enefunc%fitting_move .eq. FittingMoveREF) then
          fit_work(1:3,1:enefunc%num_atoms_ref) = &
    ...
    do i = 1, numatoms(list(1,inum))
      ...
      if (enefunc%rpath_sum_mf_flag .and. enefunc%rpath_pos_func > 0) then
        if ((enefunc%fitting_method .eq. FittingMethodTR_ROT) .or. &
            (enefunc%fitting_method .eq. FittingMethodXYTR_ZROT)) then
  ...
  subroutine compute_energy_flatbottom_restraint(enefunc, boundary, coord, inum, &
                                          calc_force, force, virial, &
                                          efb)
