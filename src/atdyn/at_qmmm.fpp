...
100 continue
  ...
  subroutine qm_check_input_gaussian_frwf(qmmm, setup_error)
    ...
    logical                     :: defined_gen, defined_iop6, defined_iop4,  &
                                   defined_coord, defined_charge, defined_elfield
    ...
    do
      ...
      if(.not.is_route_section .and. (index(string, '#') /= 0)) is_route_section = .true.
    ...
    if(.not. defined_coord .or. .not. defined_charge .or. &
       .not. defined_coord .or. .not. (defined_iop6 .and. .not.qmmm%ene_only)) then
      write(MsgOut,'(a)') 'ERROR>>> Fatal error in '//trim(qmmm%qmcnt)
      if(.not.defined_iop6 .and. .not.qmmm%ene_only) &
...
200 continue
  ...
  subroutine qm_read_output_gaussian_frwf(molecule, qmmm, qm_energy, qm_error)
    ...
    logical             :: found_energy, found_force, found_charge, &
                           found_dipole, found_selfe, found_efield
    integer             :: file1
    integer             :: i, j
    integer             :: nsta, nend
    integer             :: address
    ...
    real(wp), pointer   :: qm_force(:,:), mm_force(:,:), la_force(:,:), qm_charge(:), la_charge(:)
  ...
  contains
    ...
    subroutine pass_value(natom_mm, istart, iend, jpos_save, field_tmp, mm_force)

      ! formal arguments
      integer, intent(in)    :: natom_mm, istart, iend
...
200 continue
  ...
  subroutine generate_linkatoms(molecule, psf, qmmm)
    ...
    integer                  :: i, j, k, l, ista, iend, iatom, ibond, igrp, natom_grp, iatom_rm
    integer                  :: lkatm
