...
contains
  ...
  subroutine fft3d_1d_alltoall(qdf, qdfxyz, qdfyxz, qdfzyx, qdfzxy, qdfyxz2, &
                         qdfxyz_work, qdfyxz_work, qdfzyx_work, qdfzxy_work, &
                         qdfzxy_work2, qdfzxy_work3, work1, work2,           &
                         nlocalx, nlocaly, nlocalz, nlocalx1, x_local1,      &
                         nx, ny, nz, nprocx, nprocy, nprocz, id, nthread,    &
    ...
    complex(wp),      intent(inout) :: qdfzxy_work3(nz,nlocalx1,nlocaly)
    complex(wp),      intent(inout) :: work1(*), work2(*)
    integer,          intent(in)    :: nlocalx, nlocaly, nlocalz
    integer,          intent(in)    :: nlocalx1
    integer,          intent(in)    :: x_local1
    ...
    ! local variables
    integer*8         :: plan
    integer           :: ix, iy, iz, ixs, ixe, iys, izs, niz, niy, izss, iyss
    integer           :: ix_start1, ix_end1, iy_start, iy_end, iz_start, iz_end
    integer           :: i, k, k1, iproc, ierror
...
!       end do
...
#ifdef _SINGLE
        call sfftw_execute_dft(plan, qdfzxy_work2(1,ix,iy), work2)
#else
        call dfftw_execute_dft(plan, qdfzxy_work2(1,ix,iy), work2)
#endif 
        do k = 1, nz
  ...
  subroutine fft3d_2d_alltoall(qdf, qdfyxz_work, work1, work2,          &
    ...
    integer           :: ix, iy, iz, ixs, ixe, iys, izs, niz, niy, izss, iyss
    integer           :: ix_start1, ix_end1, iy_start, iy_end, iz_start, iz_end
    integer           :: i, k, ii, kk, k1, iproc, ierror
  ...
  subroutine bfft3d_1d_alltoall(qdf, qdf_real, qdfzxy, qdfyxz, qdfzyx,  &
                          qdfzxy_work, qdfyxz_work, qdfzyx_work, work1, &
                          work2, nlocalx, nlocaly, nlocalz, nlocalx1,   &
                          x_local1, nx, ny, nz, nprocx, nprocy, nprocz, &
    ...
    complex(wp),      intent(inout) :: qdfzxy_work(nlocalz,nlocalx1,nlocaly,*)
    complex(wp),      intent(inout) :: qdfyxz_work(nlocaly,nlocalx1,nlocalz,*)
    complex(wp),      intent(inout) :: qdfzyx_work(nz,niy,nlocalx1,*)
    complex(wp),      intent(inout) :: work1(*), work2(*)
    integer,          intent(in)    :: nlocalx, nlocaly, nlocalz
    integer,          intent(in)    :: nlocalx1
    integer,          intent(in)    :: x_local1
    ...
    real(wp)          :: work3(nx)
    integer*8         :: plan
    integer           :: ix, iy, iz, iys, iyss, izs, izss, k1
      ...
      do iy = id+1, niy, nthread
          ...
          do i = 1, nz, 16
            do iz = i, min0(i+15,nz)
              work1(iz) = qdfzyx_work(iz,iy,ix,1)
            end do
          end do
        ...
        do iproc = 1, nprocz
          ...
          iyss = (iproc-1)*niy + iy
          do iz = 1, nlocalz
            qdfzxy_work(iz,ix,iyss,1) = work1(k+iz)
          end do
        end do
#endif
    ...
    do iz = iz_start+id, iz_end, nthread
      ...
      do ix = 1, x_local1
        ...
        do iproc = 1, nprocy
          ...
          izss = (iproc-1)*niz + izs
          do iy = 1, nlocaly
            qdfyxz_work(iy,ix,izss,1) = work1(k+iy)
          end do
        end do
#endif
      end do
  ...
  subroutine bfft3d_2d_alltoall(qdf, qdf_real, qdfyxz_work, work1, work2, &
    ...
    integer*8         :: plan
    integer           :: ix, iy, iz, iys, iyss, izs, izss, k1
    integer           :: niy, niz, iy_start, iy_end, iz_start, iz_end
  ...
  subroutine fft3d_1d_alltoall_x_serial(plan_fx, plan_fy, plan_fz, &
    ...
    ! local variables
    integer           :: ix, iy, iz, ixs, ixe, iys, izs, izss, iyss
    integer           :: nix, niy, niz
