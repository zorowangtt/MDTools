...
contains
  ...
  subroutine read_rst_binary(file, rst)
    ...
    if (find_tag(file, 'energy')) &
      call read_data_real(file, 'energy', rst%energy)

    if (find_tag(file, 'delta_r')) &
      call read_data_real(file, 'delta_r', rst%delta_r)

    ! MIN and MD
    if (find_tag(file, 'box_size_x')) &
      call read_data_real(file, 'box_size_x', rst%box_size_x)

    if (find_tag(file, 'box_size_y')) &
      call read_data_real(file, 'box_size_y', rst%box_size_y)

    if (find_tag(file, 'box_size_z')) &
      call read_data_real(file, 'box_size_z', rst%box_size_z)

    if (find_tag(file, 'coord_x')) then
      call alloc_rst(rst, RestartAtom, rst%num_atoms)
      allocate(v(rst%num_atoms))

      call read_data_real_array(file, 'coord_x', (/rst%num_atoms/), v(1:rst%num_atoms))
      do i = 1, rst%num_atoms
        rst%coord(1,i) = v(i)
      end do

      call read_data_real_array(file, 'coord_y', (/rst%num_atoms/), v(1:rst%num_atoms))
      do i = 1, rst%num_atoms
        rst%coord(2,i) = v(i)
      end do

      call read_data_real_array(file, 'coord_z', (/rst%num_atoms/), v(1:rst%num_atoms))
      do i = 1, rst%num_atoms
        rst%coord(3,i) = v(i)
      end do
    ...
    if (find_tag(file, 'thermostat_momentum')) &
      call read_data_real(file, 'thermostat_momentum', rst%thermostat_momentum)

    if (find_tag(file, 'barostat_momentum')) &
      call read_data_real_array(file, 'barostat_momentum', (/3/), rst%barostat_momentum(1:3))

    if (find_tag(file, 'velocity_x')) then
      call alloc_rst(rst, RestartAtom, rst%num_atoms)
      allocate(v(rst%num_atoms))

      call read_data_real_array(file, 'velocity_x', (/rst%num_atoms/), v(1:rst%num_atoms))
      do i = 1, rst%num_atoms
        rst%velocity(1,i) = v(i)
      end do

      call read_data_real_array(file, 'velocity_y', (/rst%num_atoms/), v(1:rst%num_atoms))
      do i = 1, rst%num_atoms
        rst%velocity(2,i) = v(i)
      end do

      call read_data_real_array(file, 'velocity_z', (/rst%num_atoms/), v(1:rst%num_atoms))
      do i = 1, rst%num_atoms
    ...
    if (rst%sph_pot) then
      ...
      call read_data_real_array(file, 'radius', (/rst%nfunctions/), rst%radius)
      call read_data_real_array(file, 'center', (/3,rst%nfunctions/), rst%center)
      call read_data_real_array(file, 'const', (/rst%nfunctions/), rst%const)
      call read_data_integer_array(file, 'exponent', (/rst%nfunctions/), rst%exponent)
      call read_data_real(file, 'fix_layer', rst%fix_layer)
...
30    if (random_size > 0) then
  ...
  subroutine write_rst_binary(file, rst)
    ...
    ! MIN
    call write_data_real(file, 'energy', rst%energy)
    call write_data_real(file, 'delta_r', rst%delta_r)

    ! MIN and MD
    call write_data_real(file, 'box_size_x', rst%box_size_x)
    call write_data_real(file, 'box_size_y', rst%box_size_y)
    call write_data_real(file, 'box_size_z', rst%box_size_z)

    if (allocated(rst%coord)) then
      call write_data_real_array(file, 'coord_x', (/rst%num_atoms/), rst%coord(1,:))
      call write_data_real_array(file, 'coord_y', (/rst%num_atoms/), rst%coord(2,:))
      call write_data_real_array(file, 'coord_z', (/rst%num_atoms/), rst%coord(3,:))
    end if

    ! MD
    call write_data_integer(file, 'iseed', rst%iseed)
    call write_data_real(file, 'thermostat_momentum', rst%thermostat_momentum)
    call write_data_real_array(file, 'barostat_momentum', (/3/), rst%barostat_momentum(1:3))

    if (allocated(rst%velocity)) then
      call write_data_real_array(file, 'velocity_x', (/rst%num_atoms/), rst%velocity(1,:))
      call write_data_real_array(file, 'velocity_y', (/rst%num_atoms/), rst%velocity(2,:))
      call write_data_real_array(file, 'velocity_z', (/rst%num_atoms/), rst%velocity(3,:))
    end if
    ...
    if (allocated(rst%rest_reference)) then
      call write_data_real_array(file, 'rest_reference', (/2,rst%dimension,rst%nreplicas/), rst%rest_reference)
    end if
    ...
    if (rst%sph_pot) then
      ...
      call write_data_real_array(file, 'radius', (/rst%nfunctions/), rst%radius)
      call write_data_real_array(file, 'center', (/3,rst%nfunctions/), rst%center)
      call write_data_real_array(file, 'const', (/rst%nfunctions/), rst%const)
      call write_data_integer_array(file, 'exponent', (/rst%nfunctions/), rst%exponent)
      call write_data_real(file, 'fix_layer', rst%fix_layer)
