...
contains
  ...
  subroutine analyze(output, molecule, enefunc, pairlist, dynvars, boundary, &
    ...
    !! check for REST
    if ( nreplicas < 3 .and. option%rest_component ) then
      if ( main_rank ) then
        write(MsgOut,'("Warning> REST component analysis requires at least three replicas.")')
        write(MsgOut,'("Warning> So REST component analysis is turned off...")')
        write(MsgOut,'(" ")')
      end if
      option%rest_component = .false.
    end if
    ...
    if (main_rank) then
      ...
      if ( option%rest_component ) then
        write(ene_out,'("# Column 2: SOLUTE TEMPERATURE INDEX")')
        write(ene_out,'("# Column 3: SOLUTE ENERGY")')
        write(ene_out,'("# Column 4: SOLUTE-SOLVENT ENERGY")')
        write(ene_out,'("# Column 5: SOLVENT ENERGY")')
      else if ( option%component .and. .not. has_rem ) then
    ...
    do ifile = 1, num_trjfiles
      ...
      do istep = 1, trj_list%md_steps(ifile)
        ...
        if ( has_rem ) then
          ...
          !! rest energy component analysis; ingnore current replica id
          if ( option%rest_component ) then

            if ( .not. allocated(tot_rest_comp) ) then
              allocate( tot_rest_comp(3) )
            end if

            ! beta_1 / beta_0 = T0/T1
            c1 = remd%dparameters(1,remd%repid2parmsetid(1))
            c2 = c1 / remd%dparameters(1,remd%repid2parmsetid(2))
            c3 = c1 / remd%dparameters(1,remd%repid2parmsetid(3))
            c1 = 1.0_wp
            rc1 = sqrt(c1)
            rc2 = sqrt(c2)
            rc3 = sqrt(c3)
            ...
            do i = 1, 3
              ...
              call assign_condition(i, remd, ensemble, molecule, enefunc)
              ...
              tot_rest_comp(i) = dynvars%energy%total

            end do
            
            d12 = tot_rest_comp(1) - tot_rest_comp(2)
            d13 = tot_rest_comp(1) - tot_rest_comp(3)

            euv = (d13-d12*(c1-c3)/(c1-c2)) / ((rc1-rc3)-(c1-c3)/(rc1+rc2))
            euu = (d12-(rc1-rc2)*euv)/(c1-c2)
            evv = tot_rest_comp(1) - euu - euv

            if ( main_rank ) then
              write(ene_out,'(2I16,3F16.4)') &
                cur_step, prev_parmsetid, euu, euv, evv
            end if

          else if ( .not. mbar ) then
